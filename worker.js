...
async function fetchFixtures(apiToken) {
  ...
  // Include yesterday so an ongoing match
  // that started recently is not missed.
  const startDate =
    formatDate(
      new Date(
        now.getTime() -
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Sportmonks fixtures returned HTTP ${response.status}: ${errorBody}`
    );
  }
...
async function fetchLivescores(apiToken) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Sportmonks livescores returned HTTP ${response.status}: ${errorBody}`
    );
  }
